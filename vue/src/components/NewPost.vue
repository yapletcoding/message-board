<template>
  ...
  <div id="register" class="text-center">
    <form class="form-register" @submit.prevent="createPost">
      ...
      <input
        ...
        v-model="post.post_img"
      />
      <button class="btn btn-lg btn-primary btn-block" type="submit" id="create-account" v-on:submit.prevent="createPost()">
        post your post!
      </button>
    </form>
  </div>
...
export default {
  ...
  methods: {
      createPost() {
        ...
        forumService
          .postPost(this.$route.params.forum_id, newPost)
          .then((response) => {
            this.newPost = response.data;
            this.$router.push(`/forum/${this.$route.params.forum_id}`);
          })
...
#username {
  ...
  color: white;
  border-bottom: 0px 0px 0px 2px white;
  border-radius: 3px;
  padding: 5px 2em;
  transition: 0.5s;
  outline: none;
